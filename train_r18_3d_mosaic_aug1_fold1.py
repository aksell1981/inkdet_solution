...
class CFG:
    ...
    exp_name = 'train_r18_3d_mosaic'
    desc = '  crop256_16_32,stride=128, mosaic'
    # ============== pred target =============
    target_size = 1
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if  self.mode=='train' and random.randint(0,1): #probability 0.5
...
def valid_fn(valid_loader, model, criterion, device, valid_mask_gt):  # valid_xyxys
    ...
    for step, (images, labels, y, x) in tqdm(enumerate(valid_loader), total=len(valid_loader)):
        ...
        for i in range(len(y_preds)):
            # print('y_preds:',y_preds[i].shape,y1[i], y2[i], x1[i],x2[i])
