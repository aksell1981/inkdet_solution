...
class CFG:
    ...
    exp_name = 'train_r18_3d_mosaic2_exp1'
    desc = '  crop256_16_32,stride=128, mosaic-v2 (with center crop, p=0.75)'
    # ============== pred target =============
    target_size = 1
    ...
    train_aug_list = [
        ...
        A.RandomRotate90(p=0.5), #added after fr '1' training
...
def get_transforms(data, cfg):
    ...
    # print(aug)
    return aug
def crop_center(img,mask,cropx,cropy):
    y,x,_ = img.shape
    startx = x//2-(cropx//2)
    starty = y//2-(cropy//2)
...
def load_mosaic(self, index,crop):
    ...
    for i, index in enumerate(indices):
        ...
        #print('img4',img4.shape,mask4.shape)
    if crop:
        img4,mask4= crop_center(img4,mask4,h,w)
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if  self.mode=='train' and random.randint(0,3): #p=0.75
            img,mask,yx = load_mosaic(self,idx,crop=True)
        else:
...
def criterion(y_pred, y_true):
    ...
    #return 0.5 * BCELoss(y_pred, y_true) + 0.5 * (1 - fbeta(y_pred, y_true))
